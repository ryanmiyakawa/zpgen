/*
* Edit by Henry Wang 01/20/2017
* New featuer to deal with the situation that the off-axis zoneplate has to cross the
* center of the parent zoneplate:
* Add new function to decide weather the arcs is inside the region of child zoneplate,
* so now we will always loop through the parent zoneplate and then pick up the part
* that we need. Even it is loop through the parent zoneplate, we have to calculate the 
* aberration based on the center of the off-axis zoneplate, instead of the center of the 
* parent zoneplate.
* Has to think about how to deal with the new and old off-axis algorithm later....
*
* RM 08/18/2016
* Fixed errors in the OPD computation when a tilted zone plate is specified
*
* Edit by Henry Wang 08/14/2016
* Fixed the addition of tilt function about a month ago.
* The oringial tilt can be only applied to x-axis, but we should use y-axis instead.
* Since it is the orientation for SHARP and other off-axis zoneplate as well.
* No longer use cooridnateTransform and secantSolve2 to do the tilt, leave it to be done
* by secantSolve in control.
*
*
* Edit by Henry Wang 08/10/2016
* Add 1 more input value call curl_on to save time on calling joaine2 when transfer
* this code to website. curl_on = 0 will by pass those function.
*
* Move all the off-axis zone plate by Rc = p tan(CRA), so all off-axis zoneplates will
* center at the block size now, no longer its relative position in its parent zoneplate.
* Fix the feature on wrv split for off-axis zoneplate, use offaxis zoneplate diameter:
* (1+1.1)(Rc- R1) = 2.1P(tan(CRA) - tan(1)) to determine weather this offaxis zoneplate
* needs wrv split or not.
*
* Edit by Henry Wang 07/15/2016
* Address the tip and tilt of the zoneplate function
* Instead of all 3 direction, only 1 direction can be tilt for now.
* Not just tilt the zoneplate physically, you have to keep the object NA stays the same.
*
* Edit by Henry Wang 06/24/2016
* Expand the possibility to inlcude multiple field for 1 zoneplate for WRV format
* GDSII file is fine and can be fractured by commercial software.
* All the dosage range (min. and max.) stays the same among 9 different blocks.
* The only thing has to do is create extrea block file to store the trap information
* by including a coordinates transform relative to the new block.
*
* The function has to be add into TrapTransform/writeToGDSCell to check if each trap/arc is crossing the
* boundary and slice it to proper pieces.
* At the same time, we have to open 9 different files at the same time in order to mask information
* in the right field for E-beam.
*
* Edit by Henry Wang 06/23/2016
* New feature for the Free Standing zoneplate
* Add 1 more features on Free_Standing input:
* Free_Standing = 2: Switch tone (W & T). We only need the smalelr (W) one now.
*
* Opposite_Tone = 1: Print the opposite tone: move the whole set of zone number down 1 (n-1)
*                    For the whole set of the zoneplate
*
* TrapDoes power: For now, the trap does is a linear relationship. We will add a parameter to control it.
*                 TrapDoesPower default is 1 but it will become an input from user to change the relationship.
*
* Edit by Henry Wang 03/09/2016
* VS2015 detect heap corruption for this program, but there is no issue to run this same cpp code by VS2010
* on Vaio. For now the cpp code should be good to go and provide the correct GDSII file.
*
*
* Edit by Henry Wang 11/08/2015
* Instead of export the trap information into each function, we will include those functions
* inside the trap function in order to solve the memory issue and we print the information into file directly
*
* Edit by Henry Wang 09/09/2015
* Add one more feature to calculate the apodization on off-axis zoneplate: APD_Window
* Based on the relative position towards the center of the off-axis zoneplate
* We can get the apodization value at that specific region by controling its duty cycle.
* Add one feature in while loop and a subfuction to calculate the APD at specific position.
*
* Edit by Henry Wang 06/20/2015
* Add one more function to check if the trapezoid for the SmartCut function
* Satisfy the definition before putting it into the output file
*
* Edit by Henry Wang 06/06/2015
* Introduce function to generate part of the zoneplate pattern
* For mutilple patterning purposes.
*
* Edit by Henry Wang 09/30/2014
* Add free standing featrue in it.
* Include apodization feature to the on-axis zoneplate for buttress
*
* Edit by Henry Wang 07/30/2014
* Export dosage information from TrapTransform for the CAD smart cut
* Dosage = width/(dosage+ bias), then normalized it to the max value then timies 2^16 and floor it.
*
* Edit by Henry Wang 07/28/2014
* This time we introduce the "smart cut" for the CAD tool
* Insted of let CAD software cutting the trapezoid based on it's own standard,
* which might create non perfect shape as we design. We cut it first in this code to
* avoid the situation.
* We cut the trapezoids into 2 triangular and 1 rectangular. Make it has at least one
* horizontal line for a triangular, and 2 horizontal lines for rectangular.
* We will save it as txt file for now.
* The feature to include dosage will be added up to the code later.
*
* Edit by Henry Wang 07/01/2014
* Fix the bug on GDS function
* 1. The first coordinate has to be repeated, 5 vertices in one polygon
* 2. Input name won't include ".gds" anymore
* 3. Adding the function to change the layer name
*
* Edit by Henry Wang 06/27/2014
* Add new function to transfer function into GDS file directly.
*
* Edit by Henry Wang 06/09/2014
* New equation to define the radius of offaxis zoneplate for SHARP is edited by Ryan.
* The equation to define Rc is switched from f*tan(CRA) to p*tan(CRA).
* The f in the boundary equation for SHARP offaxis zoneplate has been replaced with p.
*
* Edit by Henry Wang 06/05/2014
* Enable the dark field feature (APD = 0) for offaxis zoneplate
* Enable the zernike aberration function for offaxis zoneplate
* The important part for this function is that all coordinates have to relative to
* the center of the offaxis zoneplate, not its parent zoneplate
*
* Edit by Henry Wang 05/26/2014
* Enable the phase contrast and apodization function of offaxis zoneplate into the final version.
* Get rid of the offaxis_theta because we've already include gamma into our system
* Get rid of the offaxis_R3. Use outer/innter_sigma and estimated offaxis_R2 to determine PC/APD region of off-axis zoneplate
* Ken's equation start on y-axis
*
* Edit by Henry Wang 03/05/2014
* Add gamma to rotate on the plane
* Introduce another rotation matrix into CooridnateTransform
*
* Edit by Henry Wang 03/04/2014
* Change the unit system to um to accommodate DW2K
* Add codes to provides info for scale bar
*
* Edit by Henry Wang 02/06/2014
* Change the parameter for off-axis function
* Introduce the new shape of the off-axis ZP
*
* Edit by Henry Wang 02/02/2014
* We add one new feature so that we can transfer Arc coordinate system
* to trapezoid as a option.
*
* Edit by Henry Wang 11/21/2013
* This version will include Phase contrast and APD (Angular DC)
* and also using argc and argv as input for further usage.
* For off-axis case, the region wiht PC and APD are defined by
* specific pattern, which means need the CT_array2 and nandtheta_array2
*
* Edit by Henry Wang 9/28/2013
* This version will include off-axis feature into our code.
*
* Edit by Henry Wang 09/2013
* In version 3
* This time we will included tilt into our calculation.
* We use p & q instead of f & M to define the zone plate.
* Edit by Henry Wang 07/2013
* makeZPMx.cpp, by E Anderson and R Miyakawa, 04/2013
*
*
* This function works by solving the zone radii at each angular location.
* Angle sampling is determined on a zone-by-zone basis, and is chosen to
* meet the aberration requirement set by tolerance.  Each radius is solved
* using the Secant method, which is a discretized version of Newton's method.
*/
...
double objectiveFn(double r, double th, double * orders, int rowLen,
	...
	double zn = -zoneNum*lambda / 2;
	double rn, x_0, y_0, y_offaxis, r_new, Rc, phi,
		x_boundary, y_boundary;

	// Normalized R

	if (offaxis_aberration == 1) {
		// r has to be redefine relative to center of the offaxis zoneplate
		x_0 = r*cos(th);
		y_0 = r*sin(th);
	...
	// term from aberrations

	double Zv = 0;
	for (int k = 0; k < rowLen; k++) {
		Zv += orders[rowLen + k] * zgenpt((int)orders[k], rn, th);
	}
	double zcp = Zv*lambda / 2;

	// Geometric term
	// Transform coordinates to accommodate a non-zero tilt angle
	// Modified at 08182016

	p = p - r*sin(beta)*sin(th);
	q = q + r*sin(beta)*sin(th);
	r = r*sqrt(sin(th)*sin(th)*cos(beta)*cos(beta) + cos(th) * cos(th));

	double zgm = sqrt(p*p + r*r) - p + sqrt(q*q + r*r) - q;

	// Phase contrast term
	double zpc = 0;

	if (PC_on == 1) {
		zpc = (PC_term)*lambda / (2 * M_PI);
	}

	return zn + zgm + zcp + zpc;
...
double secantSolve(double xm1, double theta, double * orders, int rowLen,
	...
	// For now choose the beta as the angle (rotation on x-z plane)
	// Equation for the new zoneplate after tilt:
	// X = R*P/(P+R*tan(beta))

	//zpRadius = zpRadius*p / (p + zpRadius*tan(beta));
	...
	double numerator = Zout * 1.0;
	double denominator = sqrt(Xout*Xout + Yout*Yout + Zout*Zout) * 1;
	//double th = acos(numerator/ denominator);
	double th = M_PI / 2;


	//double numerator = Zout* 1;
	//double denominator = sqrt(Xout*Xout + Yout*Yout + Zout*Zout)* 1;
	//double th = acos(numerator/ denominator);

	//Includes the shift to object and image distance

	//p = p + Zout;
	//q = q - Zout;

	//zpRadius = zpRadius* sin(th);

	//find theta for objectiveFn after tilting

	double R = sqrt(Xout*Xout + Yout*Yout);
	...
	if (Xout >= 0 && Yout >= 0)
		...
		theta0 = M_PI - asin(Yout / R);
	else if (Xout <= 0 && Yout < 0)
		theta0 = M_PI + asin(abs(Yout / R));
	else
		theta0 = 2 * M_PI - asin(abs(Yout / R));

	double x_test0 = R;
	double x_test1 = 1.01*R;


	//Instead of recall the function again, we use while to do the iteration

	while (currentIter <= maxIter) {

		// Compute the value of f(xm1) and f(xm2)

		double fxm1 = objectiveFn(x_test0, theta0, orders, rowLen, zoneNum,
			p, lambda, q, zpRadius, PC_term, PC_on, offaxis_aberration,
			CRA, NA_C, beta);
		double fxm2 = objectiveFn(x_test1, theta0, orders, rowLen, zoneNum,
			p, lambda, q, zpRadius, PC_term, PC_on, offaxis_aberration,
			CRA, NA_C, beta);

		// Compute new guess

		double R0 = x_test0 - fxm1*(x_test0 - x_test1) / (fxm1 - fxm2);
		...
		if (abs(R0 - x_test0) < tolX) {
			return R0 / sin(th);
			break;
		}

		//doesn't pass the two criteria, put it back to do the iteration
...
// secanSolve2 is used to sovle ZoneNum for off-axis situation, which we known r in advance.
double secantSolve2(double xm1, double theta,
	double * orders, int rowLen,
	double p, double lambda,
	double q, double alpha,
	double beta, double gamma, double zpRadius0,
	int offaxis_aberration,
	double CRA, double NA_C) {
	...
	//double zpRadius = zpRadius*p / (p + zpRadius*tan(beta));

	//Switch sign depends on the X
	if (Xin > 0) {
		beta2 = beta;
	}
	else if (Xin < 0) {
	...
	Zout = -Yin*sin(beta2);
	//Xout = Xin;
	//Yout = Yin*p / (p - Yin*tan(beta2));
	//Zout = -Yout*tan(beta2);

	//double zpRadius = (zpRadius0/p)* (p - Zout);
	double zpRadius = zpRadius0;
	...
	//double zpRadius = zpRadius* sin(th);

	/*

	// Doing coordinate transform without using matrix multiplication

	//Xout = Xin* cos(beta);
	//Yout = 1*Xin*sin(alpha)* sin(beta)+ Yin*cos(alpha);
	//Zout = -1*Xin*cos(alpha)*sin(beta)+ Yin*sin(alpha);

	Xout = Xin* cos(beta)* cos(gamma) - Yin*cos(beta)*sin(gamma);
	Yout = Xin*(sin(alpha)* sin(beta)* cos(gamma)+ cos(alpha)* sin(gamma))+ Yin* (cos(alpha)* cos(gamma)- sin(alpha)*sin(beta)*sin(gamma));
	Zout = Xin*(sin(alpha)* sin(gamma)-cos(alpha)*sin(beta)*cos(gamma))+ Yin*(sin(alpha)*cos(gamma)+ cos(alpha)*sin(beta)*sin(gamma));

	//Find the angle between vector and +z axis (0 0 1)
	...
	double zpRadius = zpRadius0* sin(th);

	//fin theta for objectiveFn after tilting
	*/
	double R = sqrt(Xout*Xout + Yout*Yout);
	...
	double r0 = r*sin(th);
	double x_0, y_0, Rc, y_offaxis, r_new, zpRadus, rn, phi, x_boundary, y_boundary;

	if (offaxis_aberration == 1) {
		// r has to be redefine relative to center of the offaxis zoneplate
		// Use ken's eq. to define the zpRadius, which is the boundary of offaxis zoneplate
		// It only need the phi relative the center of the offaxis zoneplate (0, Rc) and all other
		// input parameter to define the coordinates.
...
//double rn = r0/zpRadius;

// rn might larger than 1, which means the region doesn't have ZP over there.
...
// // term from aberrations:\
	...
	double Zv = 0;
	for (int k = 0; k < rowLen; k++) {
		Zv += orders[rowLen + k] * zgenpt((int)orders[k], rn, theta0);
	}
	double zcp = Zv*lambda / 2;
	// double zcp = 0;
	// geometric term:\
...
double drforArc(int n, double theta, double endp, double lambda, double p,
	double q, double alpha, double beta, double gamma, double zpRad, double * orders,
	int numA, double PC_term, int PC_on, int offaxis_aberration, double CRA, double NA_C) {
	...
	for (int j = 0; j < 11; j++) {
		double thetaj = theta + j*th;

		x0 = sqrt(n * lambda * (p*q) / (p + q));
		rs = secantSolve(x0, thetaj, orders, numA, n, p, lambda, q, alpha, beta, gamma, zpRad, 1,
			MAXITER, TOLX, PC_term, PC_on, offaxis_aberration, CRA, NA_C);
		x0 = sqrt((n + 1) * lambda * (p*q) / (p + q));
		re = secantSolve(x0, thetaj, orders, numA, n + 1, p, lambda, q, alpha, beta, gamma, zpRad, 1,
			MAXITER, TOLX, PC_term, PC_on, offaxis_aberration, CRA, NA_C);

		drM += (re - rs);
	}
	double output = 1000 * drM / 11;
	return output;
...
float * Control(double* nandtheta_array, double* nandtheta_array2, double* nandtheta_array3, double lambda, double p, double q, double alpha, double beta, double gamma,
	...
	double zTol, int nandtheta_array_size, int nandtheta_array_size2, int nandtheta_array_size3, double PC_term, double APD, double bias, long zpID,
...
float * HWcustomZP(double zTol, double lambda, double p, double q, double alpha, double beta, double gamma,
	...
	double PC_term, double APD, double bias, long zpID, int offaxis_aberration, double CRA,
...
void writeToGDSCell(float* xArray, double arrayR, double arrayC, char** argv_test, long zpID, int curl_on) {
...
void writeToGDSCell_Trap(float* xArray, long arrayR, long arrayC, char** argv_test, long zpID, double first, FILE* fp, int curl_on) {
	...
	for (long k = 0; k < arrayR; k++) {
		//			fprintf(fp, "b{1 xy(");
		
		for (long m = 0; m < arrayC; m++) {
...
// Gateway function
void mexFunction(float* xArray, int arrayR, int arrayC, char** argv_test, long zpID,
	double first, FILE* op, int name_size, int curl_on)
{
	...
	for (long k = 0; k < arrayR; k++) {
		...
		for (long m = 0; m < arrayC; m++) {
			long num = xArray[arrayR*m + k] * 10000;
...
void CAD_tool(float* xArray, long long numTrap, long arrayC, char** argv_test, long zpID, int block_size, int wrv_split,
	int block_index, long long & totalPoly, double maxDose, double minDose, double first, double* first_WRV_Split, FILE* fp, FILE** fp_wrv) {
	...
	VectorXf vertices_coord3(8);
	VectorXd SmartCut;
	long vertices_coord2[8];
	long vertices_test[8];
	long vertices_dose;
	VectorXf vertices_ycoord(4);
	long vertices_ycoord_test[4];
	...
	long Trap[8];
	long Tri[6];
	long vertices_coord2_y[4];
	long extra_x, extra_y;
	int test_situation;
	//Unit nm
		...
		while (piece == 1) {
			...
			if (pass == 1) {
				...
				for (long m = 0; m < 6; m++) {
					...
					test[m] = (long long)ceil(((double)Smart_Cut2[m] / (double)block_size)*block_size / 10 / (double)pixel_size);
				}
				...
				if (abs(test[1] - test[3]) > 1) {
					...
					for (long m = 0; m < 6; m++) {
						//double test = ((double) xArray[k*7+ m]/ (double)block_size)*block_size/5;
						fprintf(fp, "%lld", (long long)ceil(((double)Smart_Cut2[m] / (double)block_size)*block_size / 10 / (double)pixel_size));
						if (m != 5)
							fprintf(fp, " ");
					}
				...
				for (long m = 0; m < 6; m++) {
					//double test = ((double) xArray[k*7+ m]/ (double)block_size)*block_size/5;
					test[m] = (long long)ceil(((double)AfterCut[m + 8] / (double)block_size)*block_size / 10);
				}
				...
				if (abs(test[1] - test[3])> 1) {
					...
					for (long m = 0; m < 6; m++) {
						//double test = ((double) xArray[k*7+ m]/ (double)block_size)*block_size/5;
						fprintf(fp, "%lld", (long long)ceil(((double)AfterCut[m + 8] / (double)block_size)*block_size / 10 / (double)pixel_size));
						if (m != 5)
							fprintf(fp, " ");
					}
					//fprintf(fp, " %d", test_situation);
...
void writeToGDT_CAD(long long* xArray, long long totalpoly, double arrayC, char** argv_test, long zpID, double maxDose, double minDose, int block_size, int curl_on) {
	...
	fprintf(fp, "vepdef 20 %d %d\n", block_size / 10, block_size / 10);
	for (long long k = 0; k < totalpoly; k++) {
		fprintf(fp, "trap/");
		//dosage
		fprintf(fp, "%lld ", xArray[k * 7 + 6]);

		for (long m = 0; m < 6; m++) {
			//double test = ((double) xArray[k*7+ m]/ (double)block_size)*block_size/5;
			fprintf(fp, "%lld", (long long)floor(((double)xArray[k * 7 + m] / (double)block_size)*block_size / 10));
			if (m != arrayC - 1)
				fprintf(fp, " ");
		}
		fprintf(fp, "\n");
...
void TrapTransform(float* ReproduceArc_array, long &numTraps, int totalArc, double zTol, double bias, double & max_dose, double & min_dose, int File_format, int wrv_split,
	double TrapDoesPower, char** argv_test, long zpID, int block_size, long long & totalpoly, int curl_on) {
	...
	long * trapsPerArc = new long[totalArc];
	long * cumulativeTrapsPerArc = new long[totalArc];
	...
	for (int index = 0; index < totalArc; index++) {
		...
		trapsPerArc[index] = (long long)ceil((dtheta / (2 * acos(-1 * (zTol*(dr) / re1 - 1)))));
			...
			if (k > 0 ) {
				if (File_format == 1) {
					...
					for (long k = 0; k < 1; k++) {
						...
						for (long m = 0; m < 8; m++) {
							long num = trapCoords_old[1 * m + k] * 10000;
							unsigned char dataBuffer[4];
							encode32(num, dataBuffer);
							for (int q = 0; q < 4; q++) {
								dataBuffer2[m * 4 + q] = dataBuffer[q];
			...
			if (k == (numSlices - 1)) {
				if (File_format == 1) {
					...
					for (long k = 0; k < 1; k++) {
						...
						for (long m = 0; m < 8; m++) {
							long num = trapCoords[1 * m + k] * 10000;
							unsigned char dataBuffer[4];
							encode32(num, dataBuffer);
							for (int q = 0; q < 4; q++) {
								dataBuffer2[m * 4 + q] = dataBuffer[q];
